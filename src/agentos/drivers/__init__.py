    ...
    *,
    ...
    Args:
        driver: Any object whose ``chat(brief, *, attachments, session_key,
            tool_subset)`` matches the :class:`BaseDriver` contract.
    ...
    # call sites. ``type: ignore`` because not every driver declares chat
    # with the same signature; runtime lookup still works.
